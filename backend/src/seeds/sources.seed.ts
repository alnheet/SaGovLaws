 ...
 */
export async function seedSources(firestore: any): Promise<void> {
    console.log("🌱 Starting sources seed...");

    let successCount = 0;
    let errorCount = 0;

    for (const source of SOURCES_CONFIG) {
        try {
 ...
 */
export async function verifySources(firestore: any): Promise<void> {
